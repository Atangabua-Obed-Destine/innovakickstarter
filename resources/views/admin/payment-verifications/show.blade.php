...
<div class="space-y-6" x-data="{ showRejectModal: false }">
    ...
    <div>
        <nav class="text-sm text-dark-400 mb-2">
            ...
            <a href="{{ route('admin.payment-verifications.index') }}" class="hover:text-white">Payment Verifications</a>
            <span class="mx-2">›</span>
            <span class="text-dark-300">Receipt Verification</span>
        </nav>
        <div class="flex items-center gap-3">
            <h1 class="text-2xl font-bold text-white">Verify Payment Receipt</h1>
            <span class="badge bg-{{ $payment->status_color }}-600/20 text-{{ $payment->status_color }}-400 border-{{ $payment->status_color }}-500/30">
                {{ $payment->status_label }}
            </span>
        </div>
        <p class="text-dark-400 mt-1">Submitted by {{ $payment->fellow->name }} on {{ $payment->created_at->format('M j, Y, g:i a') }}</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-dark-300 text-sm">Verified by {{ $payment->verifier?->name }} on {{ $payment->verified_at->format('M j, Y') }}</p>
                            </div>
                        </div>
            ...
            {{-- Payment Details --}}
            <div class="card p-6">
                <h2 class="text-lg font-semibold text-white mb-4">Uploaded Details</h2>
                <div class="space-y-4">
