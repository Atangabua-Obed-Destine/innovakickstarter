...
@section('title', "Create Milestone — {$track->name}")
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div>
        <div class="flex items-center gap-2 text-dark-400 text-sm mb-2">
            ...
            <span>/</span>
            <span class="text-primary-400">New Milestone</span>
        </div>
        <h1 class="text-2xl font-bold text-white">Create Milestone</h1>
        <p class="text-dark-400 mt-1">Add a new phase to the {{ $track->name }} curriculum</p>
    </div>
    ...
    <form action="{{ route('admin.curriculum.milestones.store', $track) }}" method="POST" class="card p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="3"
                      class="w-full bg-dark-800 border border-dark-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                      placeholder="Describe what fellows will learn and accomplish...">{{ old('description') }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="order" class="block text-sm font-medium text-dark-300 mb-2">Order</label>
                <input type="number" name="order" id="order" value="{{ old('order', $existingMilestones->count() + 1) }}" min="1"
                       class="w-full bg-dark-800 border border-dark-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500">
            </div>

            <div>
                <label for="estimated_duration_days" class="block text-sm font-medium text-dark-300 mb-2">Estimated Duration (days)</label>
                <input type="number" name="estimated_duration_days" id="estimated_duration_days" value="{{ old('estimated_duration_days') }}" min="1"
                       class="w-full bg-dark-800 border border-dark-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                       placeholder="e.g., 14">
            </div>
        </div>
        ...
        <div>
            ...
            <select name="unlock_after_milestone_id" id="unlock_after_milestone_id"
                ...
                @foreach($existingMilestones as $ms)
                    <option value="{{ $ms->id }}" {{ old('unlock_after_milestone_id') == $ms->id ? 'selected' : '' }}>
                        M{{ $ms->order }}: {{ $ms->title }}
                    </option>
                @endforeach
            </select>
            <p class="text-dark-500 text-xs mt-1">Fellows must complete the prerequisite before this milestone unlocks.</p>
        </div>
        ...
        <div class="flex items-center gap-4">
            <label class="flex items-center gap-2 cursor-pointer">
                ...
                <span class="text-dark-300 text-sm">Required for track completion</span>
            </label>
        </div>
        ...
        <div class="border-t border-dark-700 pt-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="badge_name" id="badge_name" value="{{ old('badge_name') }}"
                           class="w-full bg-dark-800 border border-dark-600 rounded-lg px-4 py-2.5 text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500"
                           placeholder="e.g., Foundation Master">
                </div>
                <div>
                    <label for="badge_icon" class="block text-sm font-medium text-dark-300 mb-2">Badge Icon (emoji)</label>
                    <input type="text" name="badge_icon" id="badge_icon" value="{{ old('badge_icon', '🏅') }}" maxlength="10"
                ...
                <div>
                    ...
                    <input type="color" name="badge_color" id="badge_color" value="{{ old('badge_color', '#8B5CF6') }}"
